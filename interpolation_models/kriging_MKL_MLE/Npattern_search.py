...
def explore(xk,delta,obj_func,alpha=2):
    ...
    for i in range(n):
        ...
        if indexmin == 0:
            ...
            xc = xkp
        fminimum = farray[indexmin]
        # if (checkPositive(xc)==True):
        #     fminimum = farray[indexmin]
        # else:
        #     delta+=0.5
        #     explore(xk,delta,obj_func)
    return fminimum,xc  
...
def hooke_jeeves(xk,delta,obj_func,eps=0.000001,itermax=500,sensitivity=0.1):
    ...
    while (steplength > eps):
        ...
        while(newf<fbefore and k < itermax): #for every successful exploratory, perform a pattern move
            ...
            tempx = newx#debug line
            [newf,newx] = explore(newx,delta,obj_func) # obtain the new points by exploratory move
            #hack line starts
            while (checkPositive(newx))==False:#debug line to ensure the output stays positive
                # delta = abs(delta)
                # delta+=0.5
                tempx+=0.1
                [newf,newx] = explore(tempx,delta,obj_func)
