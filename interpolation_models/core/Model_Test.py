...
# model = kriging_NR.Kriging(x,y,kernel='gaussian',theta0=[0.5])
# model.get_theta([0.001])
...
xt = xt_n
yt = BP.himmelblau(xt[:,0],xt[:,1])
model = kriging_NR.Kriging(xt,yt,'gaussian',theta0=[0.5,0.5])
...
n = 25
limits = [[0.05,0.15],[100,500],[1120,1680]]
xt_n = hs.halton_sequence(n,3,limits)
xt_n = np.array(xt_n)
xt_n = xt_n.T

xt = xt_n
yt = BP.waterFlow_3D(xt[:,0],xt[:,1],xt[:,2])
model = kriging_NR.Kriging(xt,yt,'gaussian',theta0=[10.0,10.0,10.0])
model.get_theta([10.0,10.0,10.0])
print("theta: {0}".format(model.theta))
si = model.sigma2
print('Sigma2: {0}'.format(si))
print("Likelihood: {0}".format(model.likelihood))
...
num = 100     
xtest = np.zeros((num,3))
xtest[:,0] = np.linspace(0.05,0.15,num)
xtest[:,1] = np.linspace(100,500,num)
xtest[:,2] = np.linspace(1120,1680,num)
y_exact = BP.waterFlow_3D(xtest[:,0],xtest[:,1],xtest[:,2])
l = len(xtest[:,0])
