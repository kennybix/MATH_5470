...
class Kriging:
    ...
    def __init__(self,x,y,kernels,theta0,eps=1.48e-08):
        self.x = x
        self.y = y
    ...
    Use bounaries to froce the weights to be between 0 and 1
    ...
    def get_theta(self, hyperparameter):
        ...
        # res = minimize(self.NLL,xk,method='SLSQP',bounds=bounds,constraints=cons,options={'disp':True})
    ...
    # def get_theta(self, hyperparameter):
    #     xk = hyperparameter
    #     res = minimize(self.cross_validate_error,xk,method='nelder-mead')
    #     optimal_hyperparameter = res.x
    #     optimal_hyperparameter = np.array_split(optimal_hyperparameter,len(optimal_hyperparameter))
    #     self.theta = self.theta0
    #     for i in range(len(self.theta0)):   
